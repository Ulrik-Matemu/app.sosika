        ...
        {loading
          ...
          : items.length > 0 ? (
            items.map(item => (
              <div
                key={item.id}
                className="min-w-[220px] snap-center bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 cursor-pointer overflow-hidden border border-gray-100"
              >
                <img
                  src={item.image_url}
                  alt={item.name}
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-between items-center mt-3">
                    <p className="text-xl text-[#00bfff] font-extrabold">
                      TZS {parseFloat(item.price).toLocaleString()}
                    </p>
                    {/* Optional: Add a quick add-to-cart button */}
                    <button className="bg-[#00bfff] text-white p-2 rounded-full shadow-md hover:bg-orange-600 transition-colors">
